...
class Api {
  ...
  public async copyMovie(movie: Movie): Promise<Movie> {
    let changed: Movie = { ...movie };

    const [err, data] = await to<Response>(
      retrieve(`${this.ep}/movies/${movie.id}/copy`, {
        method: "PUT",
        headers: new Headers({ "Content-Type": "application/json" }),
        body: JSON.stringify(movie)
      })
  ...
  public async importMovies() {
    const [err, data] = await to<Response>(
      retrieve(`${this.ep}/import`, { method: "POST" })
    );
    if (err) {
      // console.log(`reply.err(${err})`)
    } else {
  ...
  public async getDuplicates(): Promise<Movies> {
    ...
    const [err, data] = await to<Response>(
      retrieve(`${this.ep}/movies/duplicates`)
    );
    if (err) {
      // console.log(`reply.err(${err})`)
    } else {
  ...
  public async pruneMovies() {
    const [err, data] = await to<Response>(
      retrieve(`${this.ep}/prune`, { method: "POST" })
    );
    if (err) {
      // console.log(`reply.err(${err})`)
    } else {
