        ...
        <div class="row">
                ...
                <div class="c-title-shade">
                  <div class="row">
                    <div class="col-xs-12 col-sm-10">
                      <h2 class="c-title pt2 ml2">
                        {{ movie.title }} ({{ movie.year }})
                      </h2>
                    </div>
                    <div class="col-xs-12 col-sm-2 end-xs">
                      <h2 class="c-title pt2 mr2">
                        {{ runtime }} | {{ movie.imdb_rating }}
                      </h2>
                    </div>
                  </div>
                </div>
          ...
          <div class="col-xs-12 ph2 pv2">
            <div class="c-shaded ph2 pv2 mh2">
              <div class="row">
                <span class="col-xs-12 col-sm-6 c-text director">{{
                  movie.director
                }}</span>
                <span class="col-xs-12 col-sm-6 end-sm c-text">{{
                  movie.production_countries
                }}</span>
              </div>
              <div class="row between-xs mb2">
                <span class="col-xs-12 col-sm-6 c-text">{{
                  movie.actors
                }}</span>
                <span class="col-xs-12 col-sm-6 end-sm c-text">{{
                  movie.genres
              ...
              <div class="row between-xs">
                <div class="col-xs-12 col-sm-9">
                  ...
                  <span class="label secondary spacer">{{
                    movie.location
                  }}</span>
                ...
                <div class="col-xs-12 col-sm-3 end-sm">
                  <span v-if="watched" class="label success mv0 mh2">
                    <font-awesome-icon class="mh1" icon="binoculars" />
                    &nbsp;{{ lastWatched }}
                  </span>
                  <span class="label">
                    <font-awesome-icon class="mh1" icon="plus" />
                    &nbsp;{{ added }}
                  </span>
                </div>
              </div>
              ...
              <div class="row between-xs mt2">
                <div class="col-xs-12 col-sm-4 addon">
                  <input
                    ...
                    v-model.number="tmdb"
                  />
                  <button class="btn btn-default mr2" @click="fixMovie">
                    Fix
                  </button>
                  <button class="btn btn-default mr2" @click="copyMovie">
                    Copy
                  </button>
                  <div class="mr2">
                ...
                <div class="col-xs-12 col-sm-3 addon center-xs">
                  <div v-if="watched">
                    ...
                    <span class="label success mv0 mh2">{{
                      movie.count_watched
                    }}</span>
                ...
                <div class="col-xs-12 col-sm-5 addon end-sm">
                  <span v-if="hasRating" class="label success mv0 mr2">{{
                    movie.score
                  }}</span>
                  <Rating
                    :max="10"
...
export default class MovieX extends Vue {
  ...
  private fixMovie() {
    ...
    this.loading = true;
    this.$store.dispatch(constant.FIX_MOVIE, {
      id: this.movie.id,
      tmdb: this.tmdb
    });
    // }
  ...
  private copyMovie() {
    ...
    // if (this.tmdb !== this.movie.tmdb_id) {
    this.loading = true;
    this.$store.dispatch(constant.COPY_MOVIE, {
      id: this.movie.id,
      tmdb: this.tmdb
    });
    // }
  ...
  private get overview() {
    return this.movie.overview.length > 675
      ? `${this.movie.overview.slice(0, 675)} ...`
      : this.movie.overview;
  }

  private get shows() {
    return this.movie.all_watched
      .split("|")
      .map(show => format(show, "MMM DD, YYYY"));
  }
}
</script>
...
.c-title {
  color: #fff;
  text-shadow: 0 1px 0 hsla(0, 0%, 0%, 0.75), 0 0 1px hsla(0, 0%, 0%, 0.75),
    0 1px 5px hsla(0, 0%, 0%, 0.75);
}
